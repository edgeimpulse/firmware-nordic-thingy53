...
void base64_encode(const char *input, size_t input_size)
{
    int i = 0;
    int j = 0;
    unsigned char char_array_3[3];
    unsigned char char_array_4[4];
    // unsigned char char_array_5[4];
    ...
    while (input_size--) {
        ...
        if (i == 3) {
            ...
            uart_fifo_fill(uart, char_array_4, 4);
            i = 0;
        }
    }
...
bool read_encode_send(size_t address, size_t length)
{
	...
	if (!uart) {
        LOG_ERR("Failed get CDC_ACM_0 handler");
        ei_free(buffer);
        return false;
	}
    ...
    while (1) {
        ...
        base64_encode((char *)buffer, bytes_to_read);
